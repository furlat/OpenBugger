...
# The simple script can be modified using the "easy" injection method because
# it only contains simple syntax and does not have any nested code blocks.
# This means that there are fewer characters
# (e.g. quotes, brackets, braces, parenthesis) that could be the target of
# syntax errors, and the "easy" injection method, which only injects errors
# that involve replacing or removing a single character, is sufficient to
# modify the script.
print(simple_script)
...
# The medium script can be modified using the "medium" injection method because
# it contains a nested code block (the for loop in the `greet_all` function).
# This means that there are more characters
# (e.g. quotes, brackets, braces, parenthesis) that could be the target of
# syntax errors, and the "medium" injection method, which injects errors that
# involve replacing, removing, or adding a single character,
