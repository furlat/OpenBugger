...
class SyntaxInjector:
    def inject(script, characters, error_type):
        if (
            ...
            or error_type == "missing_bracket"
            or error_type == "missing_brace"
            or error_type == "missing_parenthesis"
        ):
            # remove a random instance of the character from the script
            return script.replace(random.choice(characters), "", 1)
        elif (
            error_type == "typo_quote"
            or error_type == "typo_bracket"
            or error_type == "typo_brace"
            or error_type == "typo_parenthesis"
        ):
            ...
            error_type == "extra_quote"
            or error_type == "extra_bracket"
            or error_type == "extra_brace"
            or error_type == "extra_parenthesis"
        ):
            ...
            error_type == "misplaced_quote"
            or error_type == "misplaced_bracket"
            or error_type == "misplaced_brace"
            or error_type == "misplaced_parenthesis"
        ):
            ...
            modified_list[index1], modified_list[index2] = (
                modified_list[index2],
                modified_list[index1],
            )
...
class SyntaxBug:
    def __init__(self):
        ...
        self.severity_mapping = {
            ...
            + ["typo_quote", "typo_bracket", "typo_brace", "typo_parenthesis"],
            "hard": [
                "missing_quote",
                "missing_bracket",
                "missing_brace",
            ...
            + ["typo_quote", "typo_bracket", "typo_brace", "typo_parenthesis"]
            + ["extra_quote", "extra_bracket", "extra_brace", "extra_parenthesis"]
            + [
                "misplaced_quote",
                "misplaced_bracket",
                "misplaced_brace",
    ...
    def inject(self, script, severity, num_errors):
        ...
        char_counts = {
            char: script.count(char)
            for error_type in error_list
            for char in self.errors[error_type][0]
        }
        ...
        if sum(char_counts.values()) >= num_errors:
            ...
            while errors_injected < num_errors and counter < 100 * num_errors:
                ...
                char_counts = {
                    char: modified_script.count(char)
                    for error_type in error_list
                    for char in self.errors[error_type][0]
                }
                if sum(char_counts.values()) >= num_errors - errors_injected:
                    new_modified_script = self.injector.inject(
                        modified_script, characters, error_type
                    )
            ...
            raise ValueError(
                "Not enough instances of the required characters in the script"
            )
